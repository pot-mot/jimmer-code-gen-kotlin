...
(
    ...
    `lower_case_name`            boolean      NOT NULL DEFAULT true COMMENT '启用小写命名',
    `real_fk`                    boolean      NOT NULL DEFAULT true COMMENT '启用真实外键',
    `id_view_property`           boolean      NOT NULL DEFAULT true COMMENT '生成 IdView 属性',
    `logical_deleted_annotation` varchar(500) NOT NULL COMMENT '逻辑删除注解',
    `table_annotation`           boolean      NOT NULL DEFAULT true COMMENT '生成 Table 注解',
    `column_annotation`          boolean      NOT NULL DEFAULT true COMMENT '生成 Column 注解',
    `join_table_annotation`      boolean      NOT NULL DEFAULT true COMMENT '生成 JoinTable 注解',
    `join_column_annotation`     boolean      NOT NULL DEFAULT true COMMENT '生成 JoinColumn 注解',
...
(
    ...
    `overwrite_by_raw`  boolean      NOT NULL DEFAULT FALSE COMMENT '覆盖为字面类型',
    `raw_type`          varchar(500) NOT NULL COMMENT '字面类型',
    `display_size`      bigint       NOT NULL DEFAULT 0 COMMENT '展示长度',
    `numeric_precision` bigint       NOT NULL DEFAULT 0 COMMENT '精度',
...
(
    ...
    `type`  enum ('ONE_TO_ONE','ONE_TO_MANY','MANY_TO_ONE','MANY_TO_MANY') NOT NULL COMMENT '关联类型',
