...
(
    ...
    `view_type`                  varchar(500) NOT NULL COMMENT '视图类型',
    ...
    `default_id_type`            int          NOT NULL COMMENT '默认 Id 类型',
    `generated_id_annotation`    varchar(500) NOT NULL COMMENT 'Id 注解',
    `logical_deleted_annotation` varchar(500) NOT NULL COMMENT '逻辑删除注解',
    `date_time_format_in_view`   boolean      NOT NULL COMMENT '在前端视图中进行日期格式化',
...
(
    ...
    `author`               varchar(500) NOT NULL COMMENT '作者',
    `other_annotation`     varchar(500) NULL COMMENT '其他注解',
    ...
    `page_can_query`       boolean      NOT NULL COMMENT '管理端页面中可查询',
    `page_can_add`         boolean      NOT NULL COMMENT '管理端页面中可新增',
    `page_can_edit`        boolean      NOT NULL COMMENT '管理端页面中可编辑',
    `page_can_view_detail` boolean      NOT NULL COMMENT '管理端页面中可查看详情',
    `page_can_delete`      boolean      NOT NULL COMMENT '管理端页面中可删除',
    `query_by_page`        boolean      NOT NULL COMMENT '应用分页查询',
    `remark`               varchar(500) NOT NULL COMMENT '备注',
    `created_time`         datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `modified_time`        datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    ...
    `column_id`                  bigint       NULL COMMENT '对应列',
    ...
    `type_table_id`              bigint       NULL COMMENT '类型对应表',
    `list_type`                  boolean      NOT NULL COMMENT '是否列表',
    `type_not_null`              boolean      NOT NULL COMMENT '是否非空',
    `id_property`                boolean      NOT NULL COMMENT '是否 ID 属性',
    `generated_id`               boolean      NOT NULL COMMENT '是否是生成式 ID',
    `generated_id_annotation`    varchar(500) NULL COMMENT '生成 ID 注解',
    `key_property`               boolean      NOT NULL COMMENT '是否为业务键属性',
    `key_group`                  varchar(500) NULL COMMENT '业务键组',
    `logical_delete`             boolean      NOT NULL COMMENT '是否为逻辑删除属性',
    `logical_deleted_annotation` varchar(500) NULL COMMENT '逻辑删除注解',
    `id_view`                    boolean      NOT NULL COMMENT '是否为 ID 视图属性',
    `id_view_target`             varchar(500) NULL COMMENT 'ID 视图目标',
    `association_type`           varchar(500) NULL COMMENT '关联类型',
    `long_association`           boolean      NOT NULL COMMENT '是否为长关联',
    `mapped_by`                  varchar(500) NULL COMMENT '映射镜像',
    `input_not_null`             boolean      NULL COMMENT '输入非空',
    `join_column_metas`          varchar(500) NULL COMMENT '关联列元数据',
    `join_table_meta`            varchar(500) NULL COMMENT '关联表元数据',
    `dissociate_annotation`      varchar(500) NULL COMMENT '脱钩注解',
    `other_annotation`           varchar(500) NULL COMMENT '其他注解',
    `body`                       longtext     NULL COMMENT '属性方法体',
    `enum_id`                    bigint       NULL COMMENT '对应枚举',
    `order_key`                  bigint       NOT NULL COMMENT '排序键',
    `special_form_type`          varchar(500) NULL COMMENT '特殊表单类型',
